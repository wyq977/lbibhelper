...
class TaichiMain:
    ...
    def release(self, arguments: list = sys.argv[2:]):
        ...
        parser = argparse.ArgumentParser(
            prog="ti release", description=f"{self.release.__doc__}"
        )
        args = parser.parse_args(arguments)
    ...
    def video(self, arguments: list = sys.argv[2:]):
        ...
        if self.test_mode:
            return args
        make_video(
            args.inputs, output_path=str(args.output_file), frame_rate=args.framerate
        )
    ...
    def doc(self, arguments: list = sys.argv[2:]):
        """Build documentation"""
        parser = argparse.ArgumentParser(
            prog="ti doc", description=f"{self.doc.__doc__}"
        )
        args = parser.parse_args(arguments)
    ...
    def update(self, arguments: list = sys.argv[2:]):
        ...
        parser = argparse.ArgumentParser(
            prog="ti update", description=f"{self.update.__doc__}"
        )
        args = parser.parse_args(arguments)
    ...
    def format(self, arguments: list = sys.argv[2:]):
        ...
        parser = argparse.ArgumentParser(
            prog="ti format", description=f"{self.format.__doc__}"
        )
        parser.add_argument(
            "diff",
            nargs="?",
    ...
    def build(self, arguments: list = sys.argv[2:]):
        """Build C++ files"""
        parser = argparse.ArgumentParser(
            prog="ti build", description=f"{self.build.__doc__}"
        )
        args = parser.parse_args(arguments)
    ...
    def test(self, arguments: list = sys.argv[2:]):
        """Run the tests"""
        parser = argparse.ArgumentParser(
            prog="ti test", description=f"{self.test.__doc__}"
        )
        parser.add_argument(
            "files", nargs="*", help='Test name(s) to be run, e.g. "cli"'
        )
        parser.add_argument(
            "-c",
            "--cpp",
    ...
    def task(self, arguments: list = sys.argv[2:]):
        """Run a specific task"""
        parser = argparse.ArgumentParser(
            prog="ti task", description=f"{self.task.__doc__}"
        )
        parser.add_argument(
            "taskname", help="A single task name to run, e.g. test_math"
        )
        parser.add_argument(
            "taskargs", nargs="*", help="Optional task argument(s) to run with task"
        )
        args = parser.parse_args(arguments)
    ...
    def dist(self, arguments: list = sys.argv[2:]):
        ...
        parser = argparse.ArgumentParser(
            prog="ti dist", description=f"{self.dist.__doc__}"
        )
        parser.add_argument(
            "mode",
            nargs="?",
    ...
    def lint(self, arguments: list = sys.argv[2:]):
        ...
        parser = argparse.ArgumentParser(
            prog="ti lint", description=f"{self.lint.__doc__}"
        )
